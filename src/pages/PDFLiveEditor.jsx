...
/* APP SHELL */
.pdfeditor-app{display:flex;flex-direction:column;height:100vh;overflow:hidden;background:var(--bg)}
...
.pdfeditor-btn-primary{background:var(--accent);color:#fff}
.pdfeditor-btn-primary:hover{background:#1d4ed8}
.pdfeditor-btn-success{background:var(--success);color:#fff}
...
.pdfeditor-sidebar-section{padding:16px;border-bottom:1px solid #333}
...
.pdfeditor-upload-icon{font-size:32px;margin-bottom:8px}
.pdfeditor-upload-text{font-size:12px;color:#888;font-family:'DM Mono',monospace}
.pdfeditor-upload-text strong{color:var(--accent)}

/* PAGE THUMBS */
.pdfeditor-page-thumbs{display:flex;flex-direction:column;gap:6px;padding:12px}
...
.pdfeditor-tool-sep{width:1px;height:24px;background:var(--border);margin:0 4px}
.pdfeditor-tool-btn{
...
.pdfeditor-tool-btn.active{background:var(--accent-light);color:var(--accent);border-color:var(--accent)}
...
.pdfeditor-tt-label{font-size:10px;color:#666;font-family:'DM Mono',monospace}
.pdfeditor-tt-sep{width:1px;height:20px;background:#444}
...
.pdfeditor-empty-state-icon{font-size:56px;opacity:0.3}
.pdfeditor-empty-state-title{font-size:20px;font-weight:800;color:var(--text)}
.pdfeditor-empty-state-sub{font-size:13px;color:var(--muted);font-family:'DM Mono',monospace;text-align:center;line-height:1.6}
...
.pdfeditor-progress-bar{height:100%;background:var(--accent);border-radius:2px;animation:pdfeditorProgressAnim 1.5s infinite}
@keyframes pdfeditorProgressAnim{0%{width:10%}50%{width:80%}100%{width:10%}}
...
.pdfeditor-toast{
  ...
  animation:pdfeditorToastIn 0.2s ease;box-shadow:var(--shadow-lg);
  display:flex;align-items:center;gap:8px;
}
.pdfeditor-toast.success{border-color:var(--success);color:#4ade80}
.pdfeditor-toast.error{border-color:var(--danger);color:#f87171}
@keyframes pdfeditorToastIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}
...
export default function PDFLiveEditor() {
  ...
  // FIX 1: Store page sizes per page, not just current page
  ...
  // FIX 2: Track unique edits per block, not keystrokes
  ...
  useEffect(() => {
    const handler = (e) => {
      if (!e.target.closest(".pdfeditor-text-block") && !e.target.closest(".pdfeditor-text-toolbar")) {
        setEditingId(null);
        setToolbarPos(null);
      }
    };
  ...
  const updateBlock = useCallback((id, changes) => {
    ...
    // FIX 2: Track unique block IDs modified, not keystroke count
  ...
  const exportPDF = useCallback(async () => {
    ...
    try {
      ...
      for (const [pageNumStr, blocks] of Object.entries(textBlocks)) {
        ...
        // FIX 1: Use per-page size for correct coordinate scaling
      ...
      // FIX 3: Reliable cross-browser download — append to body before clicking
  ...
  const totalEdits = Object.values(modifiedBlocks).reduce((sum, pageMap) => sum + Object.keys(pageMap).length, 0);
  const editCount = Object.fromEntries(
    Object.entries(modifiedBlocks).map(([p, map]) => [p, Object.keys(map).length])
  );
      ...
      <div className="pdfeditor-app">
        ...
        <div className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <button className="pdfeditor-btn pdfeditor-btn-success pdfeditor-btn-sm"
              onClick={exportPDF}
        ...
        <div className="pdfeditor-main">
            ...
            {!pdfDoc ? (
              <div className="pdfeditor-sidebar-section">
                ...
                <div className="pdfeditor-upload-zone"
                  ...
                  <div className="pdfeditor-upload-text"><strong>Click or drag</strong> a PDF file</div>
                </div>
              </div>
            ) : (
              <>
                ...
                <div className="pdfeditor-page-thumbs">
                  {Array.from({ length: numPages }, (_, i) => i + 1).map(p => (
                    <div key={p}
                      ...
                      <div className="pdfeditor-thumb-canvas-wrap">
                        {thumbs[p]
                      ...
                      <div className="pdfeditor-thumb-info">
                        ...
                        {editCount[p] > 0 && (
                          <div className="pdfeditor-thumb-edit-count">
                            ✏ {editCount[p]} edit{editCount[p] !== 1 ? "s" : ""}
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {/* EDITOR AREA */}
            <div className="pdfeditor-editor-area" ref={editorAreaRef}
              style={{ position: "relative" }}
              onClick={() => { setEditingId(null); setToolbarPos(null); }}>

              {!pdfDoc ? (
                <div className="pdfeditor-empty-state">
                  <div className="pdfeditor-empty-state-icon">📝</div>
                  <div className="pdfeditor-empty-state-title">PDF Live Editor</div>
                  <div className="pdfeditor-empty-state-sub">
                ...
                <div style={{ transform: `scale(${zoomScale})`, transformOrigin: "top center", transition: "transform 0.15s" }}>
                  <div className="pdfeditor-page-wrapper"
                    ...
                    <div className="pdfeditor-text-layer">
                      {currentBlocks.map(block => (
                        <div
                          key={block.id}
                          className={`pdfeditor-text-block${editingId === block.id ? " editing" : ""}${block.modified ? " modified" : ""}`}
                          style={{
                            left: block.x,
                            top: block.y,
                            width: block.w + 8,
                        ...
                        >
                          <textarea
                            ...
                            className="pdfeditor-text-block-inner"
                            value={block.text}
                            style={{
                    ...
                    {editingBlock && toolbarPos && (
                      <div className="pdfeditor-text-toolbar"
                        style={{
          ...
          {editingBlock && (
            <div className="pdfeditor-status-item">
              EDITING <span className="pdfeditor-status-accent">
                "{editingBlock.original.slice(0, 24)}{editingBlock.original.length > 24 ? "…" : ""}"
              </span>
            </div>
          )}
        </div>
